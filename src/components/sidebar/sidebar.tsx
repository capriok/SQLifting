...
const SidebarContent: React.FC<ContentProps> = ({ setSidebar }) => {
	...
	function activeLI(pathname) {
		return pathname === activePath ? styles.active_li : ''
	}
	...
	let LSsettings = localStorage.getItem('SQLifting-settings')

	const [acrd, setAcrd] = useState<Acrd>(LSsettings && JSON.parse(LSsettings) || {
		a: false,
		b: false,
		c: false,
		d: false,
...
const SidebarOptions: React.FC<OptionsProps> = ({ flipOptions, setAcrd }) => {
